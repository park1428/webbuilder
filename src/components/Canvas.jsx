...
// Inject CSS into the GrapesJS iframe to improve the editing experience
...
function Canvas({ onEditorReady, onSave }) {
  ...
  const editorOptions = useMemo(() => ({
    ...
    // Suppress all built-in GrapesJS UI panels — we build our own
    panels: { defaults: [] },
    ...
    // Inject styles into the iframe body
    canvas: {
    ...
    // Style manager — tells GrapesJS which properties to track.
    // Our RightSidebar renders its own custom UI on top; this just registers sectors.
    styleManager: {
      sectors: [
        {
          name: 'Layout',
  ...
  const handleEditorInit = useCallback((editor) => {
    ...
    editor.on('load', () => {
      // Set sensible defaults on the wrapper (page body)
    ...
    // When a component is added, scroll it into view so it's always visible.
    // Use 'instant' (not 'smooth') so the scroll completes synchronously, then
    // immediately call editor.refresh() to resync GrapesJS's highlight overlay
    // with the new scroll position — this is what prevents the blue-box misalignment.
    ...
    // Register all built-in components as GrapesJS blocks
    ...
    // Allow text editing on selection
    ...
    const scheduleSave = () => {
      ...
      saveTimer = setTimeout(() => {
        ...
        } catch (_) { /* editor torn down during navigation */ }
      }, 800);
    };
