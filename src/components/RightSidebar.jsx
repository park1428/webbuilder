...
import { analyzeDesignRemote } from '../utils/openaiDesignAdvisor';
...
function HelpIcon({ text }) {
  ...
  return (
    <Tooltip content={text}>
      <span className="sp-help-icon">?</span>
    </Tooltip>
  );
}
...
function StyleSector({ title, icon, open, onToggle, children }) {
  return (
    <div className={`sp-sector ${open ? 'sp-sector--open' : ''}`}>
      <button className="sp-sector-header" onClick={onToggle}>
...
function StylePanel({ editor, selectedComponent, openSectors, toggleSector }) {
  ...
  return (
    <div className="style-panel">
      ...
      <StyleSector title="Layout" icon="⬛" open={openSectors.Layout} onToggle={() => toggleSector('Layout')}>
      ...
      <StyleSector title="Spacing" icon="◻" open={openSectors.Spacing} onToggle={() => toggleSector('Spacing')}>
      ...
      <StyleSector title="Size" icon="⤡" open={openSectors.Size} onToggle={() => toggleSector('Size')}>
      ...
      <StyleSector title="Typography" icon="T" open={openSectors.Typography} onToggle={() => toggleSector('Typography')}>
      ...
      <StyleSector title="Background" icon="🎨" open={openSectors.Background} onToggle={() => toggleSector('Background')}>
      ...
      <StyleSector title="Border" icon="□" open={openSectors.Border} onToggle={() => toggleSector('Border')}>
      ...
      <StyleSector title="Effects" icon="✨" open={openSectors.Effects} onToggle={() => toggleSector('Effects')}>
        <div className="sp-row">
...
function RightSidebar({ editor }) {
  ...
  const [openSectors, setOpenSectors] = useState({ Layout: false, Spacing: false, Size: false, Typography: false, Background: false, Border: false, Effects: false });
  const toggleSector = (name) => setOpenSectors(prev => ({ ...prev, [name]: !prev[name] }));
  ...
  const runDesignAnalysis = useCallback(async () => {
    ...
    try {
      ...
      let aiSuggestions = [];
      try {
        const aiResult = await analyzeDesignRemote({ html, css, maxSuggestions: 3 });
        if (aiResult.error) {
          const msgs = {
            ...
            API_ERROR: aiResult.message || 'Analysis error.'
          };
          setAiError(msgs[aiResult.error] || 'Analysis unavailable.');
        } else {
          setAiError(null);
          aiSuggestions = aiResult.suggestions || [];
        }
      } catch (_) { setAiError('Unexpected analysis error.'); }
  ...
  const groupedSuggestions = visibleSuggestions.reduce((acc, s) => {
  ...
  const severityCounts = visibleSuggestions.reduce((acc, s) => {
    acc[s.severity] = (acc[s.severity] || 0) + 1;
    return acc;
  }, {});

  const isColorValue = (val) => typeof val === 'string' && /^#[0-9a-fA-F]{3,6}$|^rgb/.test(val.trim());
          ...
          <div>
            <div className="design-header">
              <div className="design-header-top">
                <div>
                  ...
                  <p className="design-subtitle">Scan your page for design issues and fix them instantly</p>
                </div>
                {undoCount > 0 && (
              ...
              >
                {analyzing
                  ? <><span className="design-spinner" />Analyzing your design...</>
                  : <><Icons.Sparkles />Run Design Check</>
                }
              </button>
              {aiError && <p className="design-ai-error">{aiError}</p>}
            </div>
              ...
              <div className="design-summary-bar">
                ...
                {severityCounts.medium > 0 && (
                  <span className="summary-pill summary-pill--medium">
                    {severityCounts.medium} warning{severityCounts.medium !== 1 ? 's' : ''}
                  </span>
                )}
                {(severityCounts.low || severityCounts.info) && (
                  <span className="summary-pill summary-pill--info">
                    {(severityCounts.low || 0) + (severityCounts.info || 0)} tip{((severityCounts.low || 0) + (severityCounts.info || 0)) !== 1 ? 's' : ''}
                  </span>
                )}
              </div>
            )}
            ...
            {suggestions === null && !analyzing && (
              <div className="rs-empty-state">
                ...
                <p className="rs-empty-hint">Run a design check to get actionable suggestions for spacing, typography, contrast, and more</p>
              </div>
            )}

            {suggestions !== null && visibleSuggestions.length === 0 && (
              <div className="design-all-good">
                <div className="design-all-good-icon-wrap">
                  <Icons.Party className="design-all-good-icon-svg" />
                </div>
                <p className="design-all-good-title">Looking great!</p>
                <p className="design-all-good-hint">No design issues found. Your page follows good design principles.</p>
              </div>
            )}
              ...
              <div className="suggestion-list">
                {Object.entries(groupedSuggestions).map(([category, items]) => (
                  <div key={category} className="suggestion-category-group">
                    <div className="suggestion-category-label">{category}</div>
                    {items.map(suggestion => {
                      const isApplied = appliedIds.has(suggestion.id);
                      const isHovered = hoveredSuggestion === suggestion.id;
                      const severityLabel = { high: 'Critical', medium: 'Warning', low: 'Tip', info: 'Info' }[suggestion.severity] || 'Info';
                      return (
                        <div
                          key={suggestion.id}
                        ...
                        >
                          <div className="suggestion-header">
                            <span className={`suggestion-badge suggestion-badge--${suggestion.severity}`}>
                              {severityLabel}
                            </span>
                                ...
                                <div className="suggestion-comparison">
                                  <div className="comparison-item comparison-before">
                                    ...
                                    <span className="comparison-value">
                                      {isColorValue(suggestion.preview.before) && (
                                        <span className="color-swatch" style={{ background: suggestion.preview.before }} />
                                      )}
                                  ...
                                  <div className="comparison-item comparison-after">
                                    ...
                                    <span className="comparison-value">
                                      {isColorValue(suggestion.preview.after) && (
                                        <span className="color-swatch" style={{ background: suggestion.preview.after }} />
                                      )}
                  ...
                  <div className="code-styles-list">
                    {getSelectedStyles().map(({ prop, value }, i) => (
                      <div key={i} className="code-style-item">
                        ...
                        {CSS_EXPLANATIONS[prop] && (
                          <Tooltip content={CSS_EXPLANATIONS[prop]}>
                            <span className="code-style-hint">?</span>
                          </Tooltip>
                        )}
                      </div>
                    ))}
