...
function App() {
  ...
  const handleSave = useCallback((payload) => {
    ...
    // Also save to current project if we have one
    if (currentProject && user) {
      // Use user-specific projects key
  ...
  const handleLogin = (userData) => {
    ...
    // Use user-specific key so each new account sees the tutorial
  ...
  const handleNewProject = () => {
    // Create a new blank project with sequential naming
    const userProjectsKey = `wb_projects_${user?.id || 'guest'}`;
    const projects = JSON.parse(localStorage.getItem(userProjectsKey) || '[]');
    const newProject = {
      id: Date.now(),
  ...
  const handleUseCommunityComponent = useCallback((component, projectId) => {
    ...
    // Queue the community component to be added once the editor is ready
